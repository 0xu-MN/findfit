...
| 5 | 의뢰 등록 위자드 — 필드 완전 설계 | 설문형/체험형 분기 · 미리보기 · 미션 플로우 |
...
## 5. 의뢰 등록 위자드 — 필드 완전 설계

> 총 6단계 / 5분 이내 완료 목표  
> **핵심 분기**: 의뢰 타입(설문형 vs 체험형)에 따라 Step 4가 완전히 달라진다.

---

### 의뢰 타입 개념 (Step 1에서 선택)

| 타입 | 설명 | 적합한 경우 | 평가단 경험 |
|------|------|-----------|-----------|
| **설문형** | 제품 설명을 읽고 질문에 바로 답변 | 아이디어 / 콘셉트 / 화면 목업 | 그 자리에서 즉시 완료 (평균 10~15분) |
| **체험형** | 앱/게임/웹을 직접 써보고 나서 평가 | 앱·게임·웹서비스 프로토타입/베타 | 체험 기간(설정 가능) 후 귀환 평가 |
...
#### Step 1 — 기본 정보 + 의뢰 타입 선택

| 항목 | 타입 | 제한 | 가이드 / 선택지 |
|------|------|------|---------------|
| 제품/서비스 이름 | 텍스트 | **최대 30자** | 핵심만 압축. 30자 제한은 엘리베이터 피치 훈련 효과 |
| 한 줄 소개 | 텍스트 | **최대 60자** | "누구를 위한 어떤 솔루션" — 리뷰어 카드에 그대로 노출 |
| 카테고리 | 멀티태그 | 최대 3개 | 앱 / 게임 / 웹서비스 / SaaS / 커머스 / 헬스 / 에듀 / 핀테크 / 푸드 / 부동산 / 기타 |
| 현재 단계 | 라디오 | 필수 | 아이디어 / 프로토타입 / 베타 / 출시 후 |
| 랜딩·소개 URL | URL | 선택 | 있으면 평가단에게 참고용 사전 공유 |
| **의뢰 타입** ⭐ | 라디오 | **필수** | **[설문형] "질문에 답변받기"** — 제품 설명 후 질문에 바로 답변 / **[체험형] "써보고 평가받기"** — 직접 사용 후 평가 |

> Step 1의 의뢰 타입 선택 결과가 Step 4 전체를 결정한다.
...
| 항목 | 제한 | 가이드 |
|------|------|--------|
| 어떤 문제를 해결하나요? | 200자 | 타겟 고객이 구체적으로 어떤 상황에서 겪는 불편인지 서술 |
| 지금 사람들은 이 문제를 어떻게 해결하나요? | 150자 | 기존 대안 및 방법 |
| 기존 대안의 한계 | 100자 | 왜 기존 방법이 불충분한지 |
| 우리 솔루션이 다른 점 | 150자 | "'더 빠르다'보다 구체적으로 — 어떤 방식으로 다른가" |
...
| 항목 | 타입 | 가이드 |
|------|------|--------|
| 연령대 | 멀티체크 | 10대 / 20대 / 30대 / 40대 / 50대 / 60대+ |
| 직군 | 멀티체크 | 직장인 / 학생 / 프리랜서 / 창업자 / 주부 / 기타 |
| 관심사 키워드 | 태그 | 최대 5개 자유 입력 (리뷰어 매칭 알고리즘 활용) |
| 타겟이 처한 상황·맥락 | 텍스트 100자 | "어떤 순간에 이 제품이 필요한지" — 예: "야근 후 퇴근길 대중교통 안에서" |
| 구매·사용 결정 요인 | 라디오 | 가격 / 편의성 / 신뢰성 / 기능 / 디자인 / 기타 |
| 지금 이 문제를 해결하는 대체 방법 | 라디오 + 텍스트 | 있음(어떤 방법?) / 없음 |

---

#### Step 4 — 검증 내용 ⭐ [설문형 / 체험형 완전 분기]

**[공통 영역 — 두 타입 모두 표시]**

| 항목 | 제한 | 가이드 |
|------|------|--------|
| 이번 검증으로 알고 싶은 것 | 100자 | "어떤 의사결정을 위해 검증하나요? 예: 출시 여부 결정 / 기능 우선순위" |
| 검증 가설 | 텍스트 | 템플릿 제공: "우리는 [타겟]이 [솔루션] 때문에 [결과]를 원한다고 가정한다" |
| **Sean Ellis Test** | 🔒 고정 | **모든 세션 자동 포함 — 삭제·수정 불가** |

---

**[설문형 전용 — 질문 설계]**

평가단이 제품 설명을 읽고 바로 답변하는 질문들을 구성한다.

| 항목 | 설명 |
|------|------|
| 질문 추가 | 최대 10개 직접 작성. Sean Ellis 1개 자동 포함 → 실제 작성 가능 최대 9개 |
| 질문 타입 | 객관식(최대 6개 선택지) / 주관식(텍스트) / 리커트 5점 척도 |
| 기본 질문 템플릿 (PSST 기반, 선택 적용 가능) | |

기본 템플릿 예시:

```
[P — 문제 인식]
· "이런 상황을 얼마나 자주 경험하시나요?" (객관식: 거의 없음~매우 자주)
· "이 불편함은 얼마나 심각한가요?" (리커트 5점)

[S — 솔루션 반응]
· "이 솔루션이 문제를 해결하는 데 얼마나 적합하다고 생각하시나요?" (리커트 5점)
· "기존 방법 대비 이 솔루션에서 가장 마음에 드는 점은?" (주관식)
· "가장 아쉬운 점이나 개선이 필요한 부분은?" (주관식)

[자동 포함 — Sean Ellis Test]
· "이 제품/서비스를 더 이상 사용할 수 없게 된다면 어떤 기분이 들겠습니까?" (고정 4지선다)
```
...
평가단이 제품을 직접 사용한 후 답변하는 구조.

| 항목 | 타입 | 필수 | 가이드 |
|------|------|------|--------|
| 체험 링크 | URL | 필수 | 앱스토어 링크 / TestFlight / 웹 URL / Google Play |
| 체험 파일 첨부 | 파일 업로드 | 선택 | APK, 압축파일 등 (최대 100MB) |
| 체험 가이드 | 텍스트 300자 | 필수 | 리뷰어가 꼭 써봐야 할 핵심 경로 안내 — 예: "1. 앱 설치 후 회원가입 → 2. '추천' 탭 둘러보기 → 3. 상품 1개 장바구니까지" |
| 예상 체험 시간 | 슬라이더 | 필수 | 5분 / 10분 / 15분 / 30분 / 1시간+ |
| 체험 완료 기한 | 라디오 | 필수 | 24시간 / 48시간 / 72시간 (기한 경과 시 미션 자동 취소) |
| 체험 후 질문 | 질문 작성기 | 최대 10개 | 설문형과 동일한 질문 타입 사용. 추가 권장 질문: "체험 중 가장 어려웠던 점은?" / "다시 사용하고 싶으신가요?" |
| 스크린샷 첨부 요청 | 토글 | 선택 | ON 시 리뷰어에게 체험 화면 캡처 1장 이상 첨부 요청 |
| Sean Ellis Test | 🔒 고정 | 자동 포함 | |

> 체험형 권장 사례금: 예상 체험 시간에 비례해 자동 제안  
> 5분 → 3,000원~ / 15분 → 8,000원~ / 30분 → 15,000원~ / 1시간+ → 25,000원~
...
| 항목 | 제한 | 비고 |
|------|------|------|
| 이미지·목업·스크린샷 | 최대 10장 (JPG·PNG·GIF) | Supabase Storage 업로드 |
| 소개 영상 URL | 선택 | YouTube / Vimeo |
| 추가 문서 | PDF 최대 2개 / 선택 | |
| 노출 설정 | 개별 토글 | 이미지·영상·문서 각각 공개 여부 설정 가능 |

> 의뢰인 실명·회사명은 설정에 관계없이 **항상 블라인드 처리** (변경 불가)

---

#### Step 6 — 평가단 선택 · 사례금 · 비용 확인

| 항목 | 타입 | 옵션 / 설명 |
|------|------|------------|
| 평가단 등급 | 라디오 | 일반 (1,500C/명) · 전문가 ★★ (2,500C/명) · 도메인전문가 ★★★ (4,000C/명) |
| 평가단 수 | 선택 | 5 / 10 / 20 / 30 / 50 / 100명 |
| 1인당 사례금 | 숫자 입력 | 최소 1,000원. 체험형은 예상 시간 기반 권장 금액 자동 표시 |
| AI 리포트 | 라디오 | 기본 포함 / 심층 분석 +5,000C |
| 완료 기한 | 라디오 | 설문형: 72시간(기본) / 48시간(+10% 할증) · 체험형: 체험 기한 + 24시간(평가 작성) |
| 잔여 캐시 | 읽기 전용 | 실시간 갱신 |
| **비용 요약** | 읽기 전용 | 아래 계산식 자동 표시 |

**비용 요약 계산 (실시간, 결제 전 마지막 확인)**

```
캐시 소모     : [평가단 등급 단가 × 평가단 수] C
                + 심층 리포트 선택 시 5,000C
잔여 캐시     : [현재 잔액 - 캐시 소모] C
─────────────────────────────────────────
사례금 소계   : [1인당 사례금 × 평가단 수] 원
FindFit 수수료: [사례금 소계 × 7.5%] 원   ← 의뢰인 부담분
부가세        : [수수료 × 10%] 원
─────────────────────────────────────────
총 현금 결제  : [사례금 소계 + 수수료 + 부가세] 원
```

> [다음: 미리보기] 버튼 → 결제 전 최종 확인 화면으로 이동

---

### 5-2. 결제 전 미리보기 (탭 전환 화면)

결제 버튼 누르기 직전, 빌더가 **두 개의 시점**으로 의뢰를 확인한다.

```
┌─────────────────────────────────────────────────┐
│  [ 내 요약 ]    [ 리뷰어에게 보이는 모습 ]          │  ← 탭 전환
├─────────────────────────────────────────────────┤
│                                                 │
│  (선택된 탭에 따라 내용 전환)                        │
│                                                 │
...
빌더가 입력한 전체 내용 + 비용 명세서.

```
제품명: OOO            타입: 설문형 / 체험형
단계: 프로토타입        카테고리: 앱, 헬스
평가단: 일반 10명       AI 리포트: 심층 포함
완료 기한: 72시간

── 질문 목록 (미리보기) ──────────────────────
1. [리커트] 이 불편함은 얼마나 심각한가요?
2. [주관식] 기존 방법 대비 가장 마음에 드는 점은?
3. [고정] Sean Ellis Test  🔒
  ... (전체 질문 펼쳐보기)

── 비용 명세서 ──────────────────────────────
캐시 차감    : 20,000C  (잔여: 60,000C)
사례금 소계  : 50,000원 × 10명 = 500,000원
수수료 7.5%  : 37,500원
부가세       : 3,750원
────────────────────────────────────────────
총 현금 결제 : 541,250원
...
#### [리뷰어에게 보이는 모습] 탭 — 블라인드 뷰

평가단이 실제로 보게 될 화면을 미리 확인. 의뢰인 정보 완전 제거.

```
┌──────────────────────────────────────────────┐
│  [앱] [헬스]  ·  프로토타입                      │
│                                              │
│  "운동 기록을 자동으로 분석해주는 앱"               │  ← 한 줄 소개 (60자)
│                                              │
│  예상 소요시간: 10~15분     사례금: 46,250원       │  ← 수수료 차감 후 금액
│  마감: 72시간 후                                │
│                                              │
│  ── 질문 목록 ────────────────────────────    │
│  1. 이 불편함은 얼마나 심각한가요? (1~5점)          │
│  2. 기존 방법 대비 가장 마음에 드는 점은?           │
│  3. 이 서비스 없어지면 얼마나 아쉬울까요? 🔒        │
│                                              │
│  의뢰인: 비공개                                 │  ← 항상 블라인드
└──────────────────────────────────────────────┘
```

> 빌더는 이 화면을 보고 "내 의뢰가 리뷰어에게 매력적으로 보이는지" 확인 후 수정 가능.  
> [수정하기] 클릭 시 해당 단계로 바로 이동.

---

### 5-3. 체험형 미션 플로우 (평가단 경험 설계)

체험형 의뢰가 활성화된 후 평가단이 겪는 3단계 경험.
...
평가단 앱·웹
 │
 ├── 미션 카드 노출 (푸시 or 피드)
 │    카테고리 · 체험 예상 시간 · 사례금 · 마감
 │
 ├── [미션 수락하기] 탭
 │    ├── 체험 링크 / 파일 즉시 전달
 │    ├── "N시간 안에 체험 후 평가 완료" 기한 명시
 │    ├── 캘린더 알림 추가 버튼 (선택)
 │    └── FCM 푸시 동의 (리마인더 수신용)
 │
 └── 앱 내 "체험 중 미션" 배지 표시 시작
```

#### Phase 2 — 체험 기간

```
체험 기한 - 24시간 전 : 푸시 "체험 완료하고 사례금 받아가세요!"
체험 기한 - 6시간 전  : 푸시 "마감이 얼마 안 남았어요 ⏰"
                         (완료 안 했을 때만 발송)

앱 내 선택 버튼: [체험 완료했어요] ← 필수 아님, 심리적 커밋 효과
```

#### Phase 3 — 평가 제출

```
체험 기한 내 [평가 작성하기] 버튼 활성화
 │
 ├── 스크린샷 첨부 (빌더가 요청한 경우만 표시)
 ├── 질문 답변 작성
 └── [제출하기]
      ├── 제출 즉시: "사례금 지급 처리 중" 알림
      ├── 품질 자동 검증 통과
      └── 에스크로 해제 신호 → PG사 사례금 자동 지급
```
...
```
체험 기한 경과 & 미제출
 ├── 평가단: 미션 자동 취소 + 해당 미션 접근 불가
 ├── 빌더 알림: "미완료 1명 발생 → 대체 리뷰어 자동 매칭 중"
 └── 사례금: 미제출 인원분 → 에스크로 환불 처리 (자동)
```

---

### 5-4. 블라인드 평가 설계 (공통 정책)

| 항목 | 의뢰인 화면 | 평가단 화면 | 적용 이유 |
|------|-----------|----------|---------|
| 의뢰인 실명·회사명 | ✅ 확인 | ❌ 완전 숨김 | 브랜드 편향 방지 |
| 빌더 기대 결과·가설 | ✅ 확인 | ❌ 완전 숨김 | Sycophancy 방지 |
| 카테고리·단계 태그 | ✅ 확인 | ✅ 공개 | 매칭 알고리즘 필요 |
| 제품 설명·자료 | ✅ 확인 | ✅ 공개 | 평가에 필요 |
| 체험 링크·파일 | ✅ 확인 | ✅ 수락 후 공개 | 수락 전 노출 금지 |
| Sean Ellis 질문 | ✅ 확인 | ✅ 공개 | 표준 문항, 편향 없음 |
| 다른 평가단 답변 | ✅ 리포트에서 집계 | ❌ 개별 답변 불가 | 앵커링 효과 방지 |

> **DB RLS 정책**: `evaluations` 테이블 조회 시 `builder_id`, `company_name` JOIN 완전 금지  
> Row Level Security로 DB 레벨에서 강제 적용
...
│   │   │   ├── preview/page.tsx      # 결제 전 미리보기 (크리에이터 뷰 / 리뷰어 뷰)
│   │   │   └── steps/
│   │   │       ├── Step1BasicInfo.tsx         # 기본정보 + 의뢰 타입 선택
│   │   │       ├── Step2ProblemSolution.tsx
│   │   │       ├── Step3TargetCustomer.tsx
│   │   │       ├── Step4Survey.tsx            # 설문형 — 질문 설계
│   │   │       ├── Step4Experience.tsx        # 체험형 — 체험 링크·가이드·질문
│   │   │       ├── Step5Attachments.tsx
│   │   │       └── Step6PaymentSelect.tsx     # 사례금 + 비용 요약
│   │   ├── requests/[id]/page.tsx
│   │   └── reports/[id]/page.tsx
│   ├── evaluator/
│   │   ├── dashboard/page.tsx
│   │   ├── available/page.tsx
│   │   ├── mission/[id]/page.tsx          # 체험형 미션 상세 (Phase 1~3)
│   │   ├── review/[id]/page.tsx
│   │   ├── history/page.tsx
│   │   └── profile/page.tsx
│   └── admin/
...
│   ├── builder/
│   ├── evaluator/
│   │   ├── MissionCard.tsx             # 체험형 미션 카드 (피드 노출용)
│   │   └── MissionProgress.tsx         # 체험 Phase 진행 상태 표시
│   └── shared/
├── lib/
│   ├── supabase/
│   │   ├── client.ts
...
-- 의뢰
requests (id, builder_id, title, description,
          stage: 'idea'|'prototype'|'beta'|'post_launch',
          category TEXT[],
          request_type: 'survey'|'experience',   -- 설문형 vs 체험형
          -- 체험형 전용 필드 (설문형은 NULL)
          experience_url TEXT,                    -- 앱스토어 링크 / 웹 URL
          experience_guide TEXT,                  -- 체험 가이드 (300자)
          experience_duration_minutes INT,        -- 예상 체험 시간 (분)
          experience_deadline_hours INT,          -- 체험 완료 기한 (24/48/72)
          require_screenshot BOOLEAN DEFAULT FALSE,
          -- 공통
          target_count INT, evaluator_grade TEXT, honorarium_per_person INT,
          status: 'pending'|'active'|'completed'|'cancelled',
          cash_spent INT, escrow_id UUID, created_at TIMESTAMPTZ)

-- 평가 (블라인드: evaluator는 builder 정보 접근 불가)
evaluations (id, request_id, evaluator_id, answers JSONB,
             sean_ellis_score: 1|2|3|4,
             mission_status: 'accepted'|'experiencing'|'submitted'|'expired',  -- 체험형 전용 상태
             screenshot_urls TEXT[],   -- 체험형 스크린샷 첨부
...
12. **의뢰 타입 분기** — `request_type === 'experience'`인 경우 Step4Experience.tsx 렌더링. 두 컴포넌트는 완전히 분리하고 공통 로직은 `lib/wizard/questions.ts`에서 관리
13. **체험형 미션 상태** — `mission_status` 전환은 서버 사이드에서만 처리 (accepted → experiencing → submitted | expired). 클라이언트에서 직접 상태 변경 금지
...
*FindFit claude_findfit.md v5.0 — 2026-05-25*
*캐시 마진 + 사례금 수수료 이중 레버 구조 / Three.js 오렌지 브랜드 랜딩 / 손익분기 53건*
*v5.0 추가: 의뢰 등록 위자드 완전 필드 설계 / 설문형·체험형 타입 분기 / 결제 전 미리보기 / 체험형 미션 플로우*
