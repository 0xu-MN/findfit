...
${context.wizardStep ? `[지금 사용자가 등록 마법사 화면에서 작성 중인 프로젝트 — 현재 "${context.wizardStep.stepLabel}" 단계]
지금까지 이 마법사에 입력된 프로젝트 전체 내용:
${context.wizardStep.fieldsHint ?? '(아직 입력 없음)'}
사용자가 "이 프로젝트에 대해 더 물어볼 게 있을까", "이 부분 어떻게 쓰면 좋을까" 처럼
막연하게 도움을 요청해도, 위 프로젝트 내용을 실제로 참고해서 구체적으로 답하세요 —
어느 단계인지 처음부터 다시 묻지 말고, 이미 입력된 내용을 아는 상태에서 대화를
이어가세요. 지금 단계(${context.wizardStep.stepLabel}) 화면 자체의 문구를 물어보면
아래 [검증 문항 설계 상담](리뷰어에게 보여줄 질문 설계)과 혼동하지 말고, 이 단계
필드에 바로 넣을 수 있는 구체적인 문장 예시를 제시하세요.
` : ''}
