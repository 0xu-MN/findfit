...
type QuestionRow = { id: string; question_text: string; question_type: string; order_index: number }
...
export async function generateAndSaveReport(projectId: string, supabase: any) {
  ...
  const psf1 = questions.find((q) => q.question_text === PSF1_TEXT)
  const psf3 = questions.find((q) => q.question_text === PSF3_TEXT)
  const seanEllis = questions.find(
    (q) => q.question_type === 'sean_ellis' || q.question_text === SEAN_ELLIS_TEXT
  )
