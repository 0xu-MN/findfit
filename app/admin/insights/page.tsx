    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <main className="max-w-[1500px] mx-auto px-6 py-8 grid grid-cols-[240px_1fr_1fr] gap-5 items-start">
        ...
        <div className="flex flex-col gap-2">
          <span className="text-[10px] font-black text-[#999] uppercase tracking-wider px-1">전체 글</span>
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[#1D1C1C]/8 p-6 text-center">
              <p className="text-[11px] font-bold text-[#999]">작성된 글이 없습니다</p>
            </div>
          ) : (
            posts.map((p) => (
              <div
                key={p.id}
                onClick={() => startEdit(p)}
                className={`bg-white rounded-xl border p-3 flex flex-col gap-1 cursor-pointer transition-colors ${
                  editingId === p.id ? 'border-[#F77019] bg-[#F77019]/5' : 'border-[#1D1C1C]/8 hover:border-[#1D1C1C]/20'
                }`}
              >
                <div className="flex items-center gap-1.5">
                  <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-[#1D1C1C]/5 text-[#666]">
                    {p.type === 'feed' ? '피드' : '뉴스룸'}
                  </span>
                  {!p.published && (
                ...
                <span className="text-[11px] font-black text-[#1D1C1C] line-clamp-1">{p.title}</span>
                <div className="flex items-center justify-between">
                  <span className="text-[9px] font-bold text-[#999]">{new Date(p.created_at).toLocaleDateString('ko-KR')}</span>
                  <button
        ...
        <div className="bg-white rounded-3xl border border-[#1D1C1C]/8 p-6 flex flex-col gap-4">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-black text-[#1D1C1C] flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2 ml-auto">
              <select
                ...
                className="text-[10px] font-bold border border-[#1D1C1C]/10 rounded-lg px-2 py-1.5"
              >
                <option value="feed">피드</option>
                <option value="newsroom">뉴스룸</option>
              </select>
              <label className="flex items-center gap-1.5 text-[10px] font-bold text-[#666] cursor-pointer">
                <input
                  type="checkbox"
          ...
          <input
            ...
            placeholder="제목 없음"
            className="w-full text-[22px] font-black text-[#1D1C1C] outline-none placeholder-[#DDD] border-b border-[#1D1C1C]/5 pb-3"
          />
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] font-black text-[#999] uppercase tracking-wider">커버 이미지</span>
            <div
              ...
              className="relative flex items-center gap-3 h-14 rounded-xl border border-dashed border-[#1D1C1C]/15 bg-[#FAFAFA] px-4 cursor-pointer hover:border-[#F77019] hover:bg-[#F77019]/5 transition-colors overflow-hidden"
            >
              ...
              {uploading ? (
                ...
                <ImagePlus className="w-4 h-4 text-[#999] flex-shrink-0" />
              )}
              <span className="text-[11px] font-bold text-[#666] flex-1 truncate">
            ...
            <input
              ...
              className="text-[10px] font-bold border border-[#1D1C1C]/10 rounded-lg px-3 py-1.5"
            />
          ...
          <textarea
            ...
            rows={14}
            className="w-full text-[13px] leading-relaxed text-[#1D1C1C] outline-none resize-none placeholder-[#CCC]"
          />

          <div className="flex gap-2 pt-1 border-t border-[#1D1C1C]/5">
            <button
              onClick={handleSave}
              disabled={saving}
            ...
            {editingId && (
              <button
                onClick={resetForm}
                className="px-4 py-2.5 rounded-xl bg-[#F5F5F5] text-[#666] text-[11px] font-black hover:bg-[#EBEBEB] transition-colors mt-3"
              >
                취소
              </button>
            )}
        ...
        <div className="sticky top-6 flex flex-col gap-2">
          <span className="text-[10px] font-black text-[#999] uppercase tracking-wider px-1">실시간 미리보기</span>
          <div className="rounded-3xl border border-[#1D1C1C]/8 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.03)] overflow-hidden">
            <div className="flex flex-col gap-5 p-6">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[11px] font-black text-[#1D1C1C] truncate">{form.author || '작성자 없음'}</span>
              ...
              <h1 className="text-[18px] font-black text-[#1D1C1C] leading-snug break-words">
                {form.title || '제목 없음'}
              </h1>
