...
import { feedPosts, newsItems } from './SharedFeedPanel'
...
// 내용/로직은 안 바꿨다.
export default function HappeningSection({ basePath }: Props) {
  const router = useRouter()

  return (
    <section className="w-full flex flex-col gap-10 py-6 select-none">
      ...
      {/* ── 피드 (실제 아티클 피드) ── */}
      <Row title="피드" onMore={() => router.push(`/${basePath}/feed`)}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {feedPosts.slice(0, 4).map((post) => (
            <div
              key={post.id}
              onClick={() => router.push(`/${basePath}/feed`)}
            ...
            >
              ...
              <span className="text-[10px] text-[#999] font-medium flex items-center gap-1">
                <Clock className="w-2.5 h-2.5" />{post.readTime} · {post.author}
              </span>
            </div>
          ))}
        </div>
          ...
          {newsItems.slice(0, 4).map((item) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black px-2 py-0.5 rounded-md text-white" style={{ background: item.tagColor }}>
                  {item.tag}
                </span>
