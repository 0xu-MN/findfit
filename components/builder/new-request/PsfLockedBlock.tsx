...
import { PSF_STANDARD_QUESTIONS, type Question } from './types'
...
export default function PsfLockedBlock({ questions = PSF_STANDARD_QUESTIONS, title, desc }: Props) {
  return (
    <div className="rounded-xl border border-[#F77019]/30 bg-[#F77019]/5 p-4 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-black text-[#F77019]">
          {title ?? '아이디어 검증 핵심 질문 · 자동 포함 (삭제·수정 불가)'}
        </span>
      </div>
      <p className="text-[10px] font-bold text-[#F77019]/70 leading-relaxed">{desc ?? PSF_CORE_QUESTIONS_DESC}</p>
